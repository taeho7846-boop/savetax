    ...
    <>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {groups.length === 0 ? (
              <tr><td colSpan={10} className="text-center py-12 text-gray-400">등록된 환불 정산이 없습니다</td></tr>
            ) : groups.map(group => {
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-gray-900">{modal === "create" ? "환불 정산 추가" : "환불 정산 수정"}</h3>
            ...
            <div className="space-y-3 mb-5">
              <div>
                ...
                <input id="refund-name" type="text" defaultValue={editRow?.clientName || ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20" placeholder="거래처명" autoFocus />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-gray-500 block mb-1">대표자명</label>
                  <input id="refund-ceo" type="text" defaultValue={editRow?.ceoName || ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20" />
                </div>
                <div>
                  <label className="text-sm text-gray-500 block mb-1">담당자</label>
                  <input id="refund-assigned" type="text" defaultValue={editRow?.assignedUserName || ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20" />
                </div>
              </div>
              <div>
                <label className="text-sm text-gray-500 block mb-1">소속</label>
                <select id="refund-aff" defaultValue={editRow?.affiliation || ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input id="refund-amount" type="number" defaultValue={editRow?.withdrawalAmount || 0} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20" placeholder="0" />
                </div>
                <div>
                  <label className="text-sm text-gray-500 block mb-1">수수료</label>
                  <input id="refund-fee" type="number" defaultValue={editRow?.fee || 0} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20" placeholder="0" />
                </div>
              </div>
              <div>
                <label className="text-sm text-gray-500 block mb-1">비고 (내용설명)</label>
                <input id="refund-notes" type="text" defaultValue={editRow?.notes || ""} className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[#1a2e4a]/20" placeholder="내용 입력" />
              </div>
            </div>

    ...
    <>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-[#F2F4F6] overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {groups.length === 0 ? (
              <tr><td colSpan={11} className="text-center py-12 text-[#8B95A1]">등록된 단건 정산이 없습니다</td></tr>
            ) : groups.map(group => {
                  ...
                  {group.rows.map(item => {
                    ...
                    return (
                      <tr key={item.id} className="hover:bg-[#F5F9FF]/30 transition-colors border-b border-[#F2F4F6]">
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <input type="checkbox" checked={item.tiIssued} onChange={() => handleToggle(item.id, "tiIssued")} disabled={isPending} className="accent-[#3182F6] w-4 h-4 cursor-pointer" />
                        </td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={() => setModal(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6" onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-bold text-[#191F28]">{modal === "create" ? "단건 정산 추가" : "단건 정산 수정"}</h3>
            ...
            <div className="space-y-3 mb-5">
              <div>
                ...
                <input id="oneoff-name" type="text" defaultValue={editRow?.clientName || ""} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]" placeholder="거래처명" autoFocus />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-sm text-[#6B7684] block mb-1">대표자명</label>
                  <input id="oneoff-ceo" type="text" defaultValue={editRow?.ceoName || ""} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]" />
                </div>
                <div>
                  <label className="text-sm text-[#6B7684] block mb-1">담당자</label>
                  <input id="oneoff-assigned" type="text" defaultValue={editRow?.assignedUserName || ""} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]" />
                </div>
              </div>
              <div>
                <label className="text-sm text-[#6B7684] block mb-1">소속</label>
                <select id="oneoff-aff" defaultValue={editRow?.affiliation || ""} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input id="oneoff-amount" type="number" defaultValue={editRow?.withdrawalAmount || 0} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]" placeholder="0" />
                </div>
                <div>
                  <label className="text-sm text-[#6B7684] block mb-1">수수료</label>
                  <input id="oneoff-fee" type="number" defaultValue={editRow?.fee || 0} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]" placeholder="0" />
                </div>
              </div>
              <div>
                <label className="text-sm text-[#6B7684] block mb-1">비고 (내용설명)</label>
                <input id="oneoff-notes" type="text" defaultValue={editRow?.notes || ""} className="w-full border border-[#F2F4F6] bg-[#F9FAFB] rounded-[10px] px-3 py-2 text-sm focus:outline-none focus:border-[#3182F6]" placeholder="내용 입력" />
              </div>
            </div>

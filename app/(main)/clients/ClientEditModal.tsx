    ...
    >
      <div className="bg-white w-full max-w-xl h-full overflow-y-auto shadow-xl flex flex-col">
        ...
        <div className="flex border-b border-gray-100 px-6 shrink-0">
          ...
          >
            히스토리
            {history && <span className="ml-1 text-xs text-gray-400">({timeline.length})</span>}
          </button>
        </div>
        ...
        <div className="flex-1 px-6 py-5 overflow-y-auto">
          {!data ? (
            ...
            !history ? (
              ...
              <div className="text-center py-16 text-gray-400 text-sm">기록이 없습니다</div>
            ) : (
                  ...
                  <div key={`${item.type}-${item.id}`} className="relative">
                    ...
                    <div className="bg-gray-50 rounded-lg px-4 py-3">
                      ...
                      {item.type === "memo" && "content" in item && (
                        <p className="text-xs text-gray-500 mt-1 whitespace-pre-wrap">{item.content}</p>
                      )}

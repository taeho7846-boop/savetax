"""
건강보험 신고서 엑셀 → PDF 생성 (ZIP 직접 수정 + PDF 도장 오버레이 안함, ZIP 도장만)
"""
...
def format_biz_d6(biz):
    """000-00-00000-0 형식 (끝 0 고정)"""
    biz = biz.replace("-", "").replace(" ", "")
    if len(biz) >= 10:
        return f"{biz[:3]}-{biz[3:5]}-{biz[5:10]}-0"
    return biz + "-0"

def format_biz_d9(biz):
    """000-00-00000 형식"""
    biz = biz.replace("-", "").replace(" ", "")
    if len(biz) >= 10:
...
def format_birthday_with_gender(rn):
    """920923-1 형식 (주민번호 앞6자리-뒷첫자리)"""
    rn = rn.replace("-", "").replace(" ", "")
    if len(rn) >= 7:
        return f"{rn[:6]}-{rn[6]}"
    return rn[:6] if len(rn) >= 6 else rn
...
def modify_xlsx(input_xlsx, output_xlsx, cell_data, stamp_png_data=None, stamp_col=12, stamp_row=13):
    """xlsx ZIP 수정 (셀 + 도장)"""
    import re
        ...
        if drawing_key:
            try:
                ...
                anchor_xml = f'''<xdr:oneCellAnchor xmlns:xdr="{xdr}" xmlns:a="{a_ns}" xmlns:r="{r_ns}">
                    <xdr:from><xdr:col>{stamp_col}</xdr:col><xdr:colOff>180000</xdr:colOff><xdr:row>{stamp_row}</xdr:row><xdr:rowOff>0</xdr:rowOff></xdr:from>
...
def main():
    ...
    if ext == ".xls":
        ...
        if result.returncode != 0:
            print("ERROR: xls→xlsx 변환 실패", file=sys.stderr)
            sys.exit(1)
    ...
    try:
        # M14 = col 12, row 13 (0-indexed)
        modify_xlsx(xlsx_input, xlsx_output, cell_data, stamp_data, stamp_col=12, stamp_row=13)
    except Exception as e:
    ...
    try:
        ...
        if stamp_data:
            try:
                ...
                c.drawImage(stamp_img, 370, 135, width=50, height=50, mask="auto")
                c.save()
                packet.seek(0)

...
def create_corporate_stamp(name, size_px=300):
    ...
    draw.ellipse([margin, margin, size_px - margin, size_px - margin], outline=color, width=outer_lw)
    inner_margin = margin + outer_lw + 3
    draw.ellipse([inner_margin, inner_margin, size_px - inner_margin, size_px - inner_margin], outline=color, width=max(2, outer_lw // 2))
    ...
    draw.text(((size_px - ct_w) // 2, center - ct_h // 2), center_text, fill=color, font=center_font)
...
def format_corporate_number(num):
    """000000-0000000 형식"""
...
def main():
    ...
    ceo_name         = sys.argv[7]
    birthday         = sys.argv[8]   # 920923
    stamp_name       = sys.argv[9]
    client_type      = sys.argv[10]  # individual or corporate
    ...
    try:
        if is_corp:
            stamp_data = create_corporate_stamp(stamp_name, size_px=300)
        else:
            stamp_data = create_personal_stamp(stamp_name, size_px=300)
    ...
    try:
        ...
        ws["F11"] = format_biz_number_f11(biz_number)
        ws["N11"] = client_name
        ws["F12"] = address
        ws["F13"] = format_biz_number_f13(biz_number)
        ws["N13"] = format_corporate_number(corporate_number) if is_corp and corporate_number else ""
        ws["F15"] = ceo_name
        ws["M15"] = birthday
        print("셀 입력 완료")

        # 도장 삽입 Q31
        STAMP_CM = 2.0 if not is_corp else 2.5
        ...
        col_off = 0.5
        marker = AnchorMarker(col=16, colOff=cm_to_EMU(col_off), row=30, rowOff=cm_to_EMU(0.0))
